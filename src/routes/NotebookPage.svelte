<script lang="ts">
  ...
  const sortNotes = (notesList: Note[]) => {
    const copy = notesList.map((x) => ({
      ...x,
      updatedAt: x.updatedAt === 'No date' || !x.updatedAt ? 'December 17, 1995 03:24:00' : x.updatedAt
    }))
    copy.sort((a, b) => (new Date(a.updatedAt!) > new Date(b.updatedAt!) ? 1 : -1))
    return copy.reverse()
  }
  ...
  const loadNotes = async (nid: string) => {
    try {
      ...
      if (response && 'error' in response) {
        showNotification(response.error ?? 'Unknown error')
        loadError = response.error ?? null
      } else if (response && 'success' in response && response.success && response.notes) {
  ...
  const loadNotebooks = async () => {
    try {
      ...
      if (response && 'success' in response && response.success && response.notebooks) {
        userNotebooks = Array.isArray(response.notebooks) ? response.notebooks : []
      } else {
        userNotebooks = []
      }
    } catch (err) {
  ...
  const moveNoteHandler = async (targetNotebookId: string) => {
    ...
    const response = await moveNotes(token, targetNotebookId, selectedNotes, latestUpdatedDate)
    if (response && 'error' in response) {
      showNotification(response.error ?? 'Unknown error')
      return
    }
  ...
  <FooterView>
    <button class="btn-footer btn-contained" onclick={() => push('/notebooks')}>
      <span class="icon_text">
        <span class="material-symbols-outlined button_icon white">arrow_back</span>
        Back to Notebooks
      </span>
    </button>
  </FooterView>
  ...
  <div class="page_scrollable_header_breadcrumb_footer_list">
    ...
    {#if editNotebookMode && notebook}
      <AddNotebookForm
        method="edit"
        notebook={notebook}
        onAddNotebook={async (name, cover) => await saveEditNotebook(name, cover)}
  ...
  <FooterView>
    ...
    {#if editNotesMode && selectedNotes.length > 0 && userNotebooks && userNotebooks.length > 1}
      <button class="btn-footer btn-contained" onclick={moveNoteFormHandler}>
        <span class="icon_text">
          <span class="material-symbols-outlined button_icon white symbol_size">flip_to_front</span>
          Move
        </span>
      </button>
    {/if}
