<script lang="ts">
  ...
  const filterNotebooks = (notebooks: Notebook[]) => {
    ...
    sorted.sort((a, b) => (new Date(a.updatedAt!) > new Date(b.updatedAt!) ? 1 : -1))
...
{:else}
  <div class="page_scrollable_header_breadcrumb_footer_list">
    <NotebooksList
      error={userNotebooks && 'error' in userNotebooks ? userNotebooks.error : undefined}
      success={userNotebooks && 'success' in userNotebooks ? userNotebooks.success : false}
      notebooks={userNotebooks && 'notebooks' in userNotebooks ? userNotebooks.notebooks : []}
